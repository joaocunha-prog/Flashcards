...
comentários novos ou provas inéditas pelo Claude; o banco nasce vazio e você o popula com suas
próprias provas (Parte 2).

Todos os comandos abaixo rodam **de dentro da pasta `banco-questoes-enare/`**:
...
Isso levanta um Postgres 16 na porta **5433** do host (para não colidir com o Postgres do
`banco-questoes-uerj`, que usa 5432), com usuário `postgres`, senha `postgres` e banco
`banco_questoes_enare` — exatamente o que o `.env.example` já espera.
...
```bash
DATABASE_URL="postgresql://postgres:postgres@localhost:5433/banco_questoes_enare"
DIRECT_URL="postgresql://postgres:postgres@localhost:5433/banco_questoes_enare"
```
...
`data/taxonomy.ts` está vazio de propósito neste projeto — grave nele os temas/assuntos/tópicos do
ENARE conforme for definindo a classificação, ou pule este passo e deixe o importador criar tema e
assunto sozinho a partir das provas (Passo 5). Nenhuma questão entra aqui de qualquer forma: o banco
de questões nasce vazio de propósito, e o conteúdo entra pelas provas no passo seguinte.
...
### Passo 6 — Carregue os comentários (opcional)

Se você escreveu comentários por fora (Parte 3, Caminho A) e colocou os arquivos em
`data/comentarios/`:
...
Lê tudo que está lá e imprime a cobertura ao final. Sem arquivos ali, o comando não faz nada — os
comentários também podem ser gerados sob demanda na interface (Parte 3, Caminho B).
...
| `Can't reach database server at localhost:5433` | Postgres não está de pé | `docker compose up -d`, ou inicie seu Postgres |
...
Salve em `data/provas/`, com um nome que identifique a prova (ex.: `enare-2026.json`). A estrutura
...
| `board` | não | Padrão: `ENARE` |
| `program` | não | Ex.: área/especialidade da prova. Sem padrão |
...
```json
{
  ...
  "reference": "https://www.enare.br/caderno-2026.pdf",
  "questions": [
    {
      "number": 1,
...
curl -X POST http://localhost:3000/api/admin/import \
  ...
  --data @data/provas/enare-2026.json
```
...
Crie um arquivo em `data/comentarios/`, nomeado `enare-AAAA.json`. Se quiser dividir a prova em
lotes, use sufixos: `enare-2026-b.json`, `enare-2026-c.json` — todos são lidos.
