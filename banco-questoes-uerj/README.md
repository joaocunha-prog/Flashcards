...
- **80/20** — segue `SubthemeStat.isPareto`, calculado ao vivo a partir da incidência real do
  ...
  corte de 80% e já tenha resumo escrito. É a vitrine que cobre os 38 resumos publicados hoje.
...
Um resumo pode aparecer nas duas vitrines ao mesmo tempo, ou em nenhuma. O que **não muda** é como o
texto entra no sistema: continua sendo um arquivo em `data/resumos/`, registrado em `content.ts` e
`slugs.ts` — a marca "Selecionado" é só uma segunda forma de agrupar o que já existe, não uma
segunda forma de escrever resumo.
